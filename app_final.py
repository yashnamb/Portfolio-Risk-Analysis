pip install yfinance plotly scikit-learn scipy
...
def calculate_risk_metrics(portfolio_data, historical_data):
    ...
    var_95_dollar = var_95 * total_investment
    var_99_dollar = var_99 * total_investment
...
def main():
    ...
    with st.sidebar:
        ...
        if not selected_tickers:
            ...
            total_investment = st.number_input("Total Investment ($)", min_value=1000, value=10000, step=1000)
            st.markdown("### Allocation")
            remaining = total_investment
            
            for i, ticker in enumerate(selected_tickers[:-1]):
                max_val = remaining
                val = st.slider(f"{ticker} Investment ($)", min_value=0, max_value=max_val, value=int(total_investment/len(selected_tickers)))
                investments[ticker] = val
                remaining -= val
            
            if len(selected_tickers) > 0:
                last_ticker = selected_tickers[-1]
                investments[last_ticker] = remaining
                st.text(f"{last_ticker}: ${format_currency(remaining)}")
            
            # Add investment status message
            current_total = sum(investments.values())
            if current_total < total_investment:
                st.error(f"⚠️ Total allocated: ${format_currency(current_total)} (${format_currency(total_investment - current_total)} remaining)")
            elif current_total > total_investment:
    ...
    if st.session_state.analyzed and selected_tickers:
        ...
        with tabs[2]:
            ...
            risk_metrics = calculate_risk_metrics(portfolio_data, historical_data)
